...
import { type Swimsuit, type SwimsuitCardProps, type SortDirection } from '@/types';
...
const SwimsuitCard = React.memo(function SwimsuitCard({ swimsuit, viewMode = 'gallery' }: SwimsuitCardProps) {
  ...
  const maxStat = Math.max(
    swimsuit.stats?.pow || 0, 
    swimsuit.stats?.tec || 0, 
    swimsuit.stats?.stm || 0, 
    swimsuit.stats?.apl || 0
  );
  ...
  const SwimsuitDisplay = ({ size = 'normal' }: { size?: 'normal' | 'large' }) => {
    ...
    return (
      <div className={`${sizeClass} bg-gradient-to-b ${generateAvatar(swimsuit.character || 'Unknown')} rounded-2xl flex flex-col items-center justify-center shadow-xl border-2 border-white/20 relative overflow-hidden`}>
        {/* Swimsuit silhouette */}
        ...
        <div className="absolute bottom-1 right-1 w-6 h-6 bg-black/30 rounded-full flex items-center justify-center">
          <span className="text-xs font-bold text-white">{(swimsuit.character || 'U')[0]}</span>
        </div>
        ...
        <div className="relative p-3">
          <div className="flex items-center space-x-3 mb-2">
            ...
            <div className="flex-1 min-w-0">
              <h3 className="text-sm font-bold text-white truncate">{swimsuit.name}</h3>
              <p className="text-xs text-gray-400 truncate">{swimsuit.character}</p>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex space-x-2">
              {swimsuit.stats && Object.entries(swimsuit.stats).map(([key, value]) => (
        ...
        <div className="p-4 border-b border-dark-border/30">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-400">{swimsuit.character}</p>
            </div>
            <RarityBadge rarity={swimsuit.rarity} />
          </div>
        </div>
          ...
          <div className="px-4 pb-4">
            <div className="bg-dark-primary/40 rounded-lg p-3">
              ...
              <div className="flex flex-wrap gap-1">
                {swimsuit.skills.slice(0, 3).map((skill) => (
                  <div
                    key={skill.id}
                ...
                {swimsuit.skills.length > 3 && (
                  <div className="px-2 py-1 rounded-sm text-xs font-medium bg-gray-500/20 text-gray-300">
                    +{swimsuit.skills.length - 3}
                  </div>
                )}
              </div>
        ...
        {swimsuit.skills && swimsuit.skills.length > 0 && (
          <div className="mb-3">
            ...
            <div className="flex flex-wrap gap-1">
              ...
              {swimsuit.skills.length > 3 && (
                <div className="px-2 py-1 rounded-sm text-xs font-medium bg-gray-500/10 text-gray-400 border border-gray-500/20">
                  +{swimsuit.skills.length - 3}
                </div>
              )}
            </div>
...
export default function SwimsuitPage() {
  ...
  useEffect(() => {
    const fetchSwimsuits = async () => {
      try {
        ...
        // Properly transform API data to match component expectations
        const transformedSwimsuits = response.data.map((swimsuit: any) => ({
          ...swimsuit,
          character: swimsuit.characterId || 'Unknown', // Map characterId to character
          stats: {
            pow: swimsuit.pow || 0,
            tec: swimsuit.tec || 0,
            stm: swimsuit.stm || 0,
          ...
          skills: swimsuit.skills || [], // Use actual skills or empty array
          release: swimsuit.releaseDate || '2024-01-01', // Use actual release date
          reappear: swimsuit.reappearDate || undefined // Use actual reappear date
        }));
        
        setSwimsuits(transformedSwimsuits);
      } catch (err) {
        console.error('Failed to fetch swimsuits:', err);
  ...
  const filteredAndSortedSwimsuits = useMemo(() => {
    const filtered = multiLanguageSwimsuits.filter(swimsuit => {
      ...
      if (filter.minApl && swimsuit.stats.apl < parseInt(filter.minApl)) return false;
      if (filter.hasSkills && (!swimsuit.skills || swimsuit.skills.length === 0)) return false;
      if (filter.releaseYear && (!swimsuit.release || !swimsuit.release.includes(filter.releaseYear))) return false;
      if (filter.version && !swimsuit.id.includes(filter.version)) return false; // Simple version check
      return true;
    });
    ...
    return filtered.sort((a, b) => {
      ...
      switch (sortBy) {
        ...
        case 'total':
          ...
          break;
        case 'release':
          aValue = new Date(a.release || '1970-01-01');
          bValue = new Date(b.release || '1970-01-01');
          break;
        default:
          aValue = a.name.toLowerCase();
          bValue = b.name.toLowerCase();
  ...
  const characters = useMemo(() => [...new Set(swimsuits.map(s => s.character).filter(Boolean))].sort(), [swimsuits]);
  const rarities = useMemo(() => ['SSR', 'SR', 'R'], []);
  const releaseYears = useMemo(() => [...new Set(swimsuits.map(s => s.release?.split('-')[0]).filter(Boolean))].sort().reverse(), [swimsuits]);
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-dark-primary via-dark-secondary to-dark-primary flex items-center justify-center">
        <div className="text-white text-lg">Loading swimsuits...</div>
      </div>
    );
  }
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
          ...
          <div className="flex flex-col lg:flex-row gap-4 items-stretch lg:items-center">
            ...
            <div className="flex-1 relative">
              ...
              <input
                ...
                className="w-full bg-muted/70 backdrop-blur-sm border border-border/50 rounded-xl pl-10 pr-4 py-3 focus:outline-hidden focus:border-accent-cyan focus:ring-2 focus:ring-accent-cyan/20 transition-all placeholder-muted-foreground"
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-8">
            {paginatedSwimsuits.map((swimsuit, index) => (
              <motion.div
                key={swimsuit.id}
