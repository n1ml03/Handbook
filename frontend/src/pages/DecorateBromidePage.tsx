...
import { type EnhancedBromide, type BromideCardProps, type SortDirection } from '@/types';
...
export default function DecorateBromidePage() {
  const [bromides, setBromides] = useState<EnhancedBromide[]>([]);
  ...
  useEffect(() => {
    const fetchBromides = async () => {
      try {
        ...
        setBromides(response.data as EnhancedBromide[]);
      } catch (err) {
