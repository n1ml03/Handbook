...
import { type AccessoryCardProps, type SortDirection, type EnhancedAccessory } from '@/types';
...
export default function AccessoryPage() {
  const [accessories, setAccessories] = useState<EnhancedAccessory[]>([]);
  ...
  useEffect(() => {
    const fetchAccessories = async () => {
      try {
        setLoading(true);
        const response = await accessoriesApi.getAccessories({ limit: 1000 });
        setAccessories(response.data as EnhancedAccessory[]);
      } catch (err) {
  ...
  const filteredAndSortedAccessories = useMemo(() => {
    const filtered = multiLanguageAccessories.filter(accessory => {
      ...
      if (filterValues.version && !accessory.id.includes(filterValues.version)) return false;
    ...
    return filtered.sort((a, b) => {
      ...
      switch (sortBy) {
        ...
        case 'apl':
          ...
          break;
        case 'total':
          aValue = (a.stats?.pow || 0) + (a.stats?.tec || 0) + (a.stats?.stm || 0) + (a.stats?.apl || 0);
          bValue = (b.stats?.pow || 0) + (b.stats?.tec || 0) + (b.stats?.stm || 0) + (b.stats?.apl || 0);
          break;
        default:
          aValue = a.name.toLowerCase();
          bValue = b.name.toLowerCase();
