...
export const Card = ({ children, className }: SpacingProps) => (
...
export const Inline = ({ children, className, spacing = 'md', align = 'start', wrap = true }: { children: ReactNode; className?: string; spacing?: 'sm' | 'md' | 'lg'; align?: 'start' | 'center' | 'end' | 'between'; wrap?: boolean }) => {
  ...
  return (
    <div className={cn('flex items-center', spacingClasses[spacing], alignClasses[align], wrap && 'flex-wrap', className)}>{children}</div>
  );
};
